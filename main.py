...
def process_text_enhancements(text: str) -> str:
    """Remove emoji 🦸 e adiciona gatilho (20%)"""
    if not text:
        return text
...
async def wait_for_day_time():
    """Pausa durante modo noturno (01:00-08:00)"""
...
async def process_new_message(
    ...
    """Processa mensagem: gera afiliado + envia IMAGEM + LEGENDA"""
    
    # ✅ REGRA: SEMPRE TEM IMAGEM, se não tiver = IGNORA
    ...
    if not await has_image(bubble):
        ...
        return True  # Retorna True para salvar ID e pular
    ...
    if not meli_urls:
        ...
        return True  # Salva ID para pular
    ...
    # Processa texto (MANTÉM FORMATAÇÃO ORIGINAL)
    ...
    # ✅ PEGA IMAGEM DIRETAMENTE DO WHATSAPP (NÃO DO ML)
...
async def monitoring_loop(page_w, page_m):
    """Monitora 3 sources → 1 target (PROCESSA 1 POR VEZ)"""
