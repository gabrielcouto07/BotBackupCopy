...
def process_text_enhancements(text: str) -> str:
    ...
    # Remove apenas o emoji específico (🦸) e suas variações
    text = re.sub(r'🦸[\u200d\ufe0f♂️♀️🏻-🏿]*', '', text)
    text = re.sub(r'[\u200d\ufe0f🏻-🏿]+', '', text)
    
    # ✅ REMOVE APENAS espaços duplicados EM CADA LINHA (mantém quebras)
    lines = text.split('\n')
    cleaned_lines = []
    for line in lines:
    ...
    # Log se emoji foi removido
    if len(text) < original_len:
        logger.info(f"   🧹 Removido emoji: {SUPERHERO_EMOJI}")
    
    # Adiciona gatilho com chance configurada
...
async def process_new_message(
    ...
    """
    Processa e envia mensagem nova do source para target
    Mantém: emojis, negritos, quebras de linha, formatação original
    """
    ...
    # Processa texto: remove apenas emoji específico, mantém TUDO o resto
    enhanced_text = process_text_enhancements(text)
    
    # Substitui URLs originais pelos afiliados
    new_text = replace_urls_in_text(enhanced_text, mapping)
    
    # Formata preços antigos com tachado
    new_text = format_old_price_with_strikethrough(new_text)
    
    # ✅ MENSAGEM FINAL = TEXTO PURO (sem cabeçalhos extras)
    final_text = new_text
    ...
    # Baixa imagem original (ou screenshot como fallback)
